...
hysteresis_plot <- function(dataframe,
  ...
  if (missing(datetime)) {
    ...
    dataframe %>%
      tidyr::drop_na(!!q, !!ssc) %>%
      dplyr::mutate(Limb = ifelse(row_number() %in% c(1:which.max(!!q)),
                                  "Rising limb", "Falling limb")) %>%
      ggplot2::ggplot(ggplot2::aes(x = !!q, y = !!ssc)) +
      ggplot2::geom_path(arrow = ggplot2::arrow(length = unit(3, "mm"),
                                                ends = "last")) +
      ggplot2::geom_point(aes(color = Limb),
                          size = 2) +
      ggplot2::labs(x = expression(italic("Q")*","~m^3%.%s^-1),
                    y = expression(italic("SSC")*","~ g %.% m^-3),
                    color = "")+
      ggpubr::theme_pubclean(base_size = base_font_size,
                             ...) +
